...
use Application\Models\Coinbase\ApiKeyAuthentication;
use Application\Models\Coinbase\SimpleApiKeyAuthentication;
use Application\Models\Coinbase\OAuthAuthentication;
use Application\Models\Coinbase\ApiException;
use Application\Models\Coinbase\Rpc;
use Application\Models\Coinbase\Requestor;
...
class Coinbase{
    ...
    public static function withOAuth($oauth, $userId)
    {
      $userCoinbase = new UserCoinbaseAccess();
      $tokens = $userCoinbase->getTokens($userId);
      if($tokens['expire_time'] < time()){
        $tokens = $oauth->refreshTokens($tokens);
        $coinbaseObj = $userCoinbase->getUserCoinbaseAccess($userId);
        $coinbaseObj->exchangeArray(Array(
        	'accessToken' => $tokens['access_token'],
          'refreshToken' => $tokens['refresh_token'],
          'expires' => $tokens['expire_time']
        ));
        $coinbaseObj->saveOrUpdateAccessToken();
      }
    ...
    public function __construct($authentication, $tokens=null, $apiKeySecret=null)
    {
        ...
        if ($authentication instanceof OAuthAuthentication || $authentication instanceof ApiKeyAuthentication || $authentication instanceof SimpleApiKeyAuthentication) {
    ...
    public function buy($amount, $currency = "BTC", $agreeBtcAmountVaries=false)
    {
